...
FILMTABLE = "films_db.sqlite"


class MyWidget(QMainWindow):
    def __init__(self):
        super().__init__()
        uic.loadUi('Буду смотреть.ui', self)
        self.con = sqlite3.connect(FILMTABLE)
        self.update_films()
    ...
    def update_films(self):
        cur = self.con.cursor()
        # Получаем результат запроса, который ввели в текстовое поле
    ...
    def update_genres(self):
        ...
        # Заполняем размеры таблицы
        self.tableGenre.setRowCount(len(result))
        self.tableGenre.setColumnCount(len(result[0]))
        self.tableGenre.setHorizontalHeaderLabels(
            ['ИД', 'Название жанра'])

        # Заполняем таблицу полученными элементами
        for i, elem in enumerate(result):
            for j, val in enumerate(elem):
                self.tableGenre.setItem(i, j, QTableWidgetItem(str(val)))
    ...
    def edit_genre(self):
        rows = list(set([i.row() for i in self.tableGenre.selectedItems()]))
        ids = [self.tableGenre.item(i, 0).text() for i in rows]
        if not ids:
            self.statusBar().showMessage('Ничего не выбрано')
            return
        else:
